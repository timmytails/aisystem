...
export default function HeroCarousel() {
    ...
    return (
        <div className='relative h-[400px] sm:h-[460px] w-full overflow-hidden rounded-2xl border border-slate-700/80 bg-slate-900 shadow-2xl shadow-slate-950/50 group'>
            ...
            <button
                ...
                className='absolute left-3.5 top-1/2 z-20 -translate-y-1/2 grid h-10 w-10 place-items-center rounded-full bg-slate-950/50 text-white backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 hover:bg-[#C25E2B] active:scale-95 focus:outline-none'
            >
                <ChevronLeft size={20} />
            </button>
            <button
                ...
                className='absolute right-3.5 top-1/2 z-20 -translate-y-1/2 grid h-10 w-10 place-items-center rounded-full bg-slate-950/50 text-white backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 hover:bg-[#C25E2B] active:scale-95 focus:outline-none'
            >
                <ChevronRight size={20} />
            </button>

            {/* Frosted Bottom Dot Navigation */}
            <div className='absolute bottom-4 left-1/2 z-20 flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-slate-950/50 px-3.5 py-1.5 backdrop-blur-md border border-white/10'>
                {PET_IMAGES.map((item, idx) => (
                    <button
                        key={item.id}
