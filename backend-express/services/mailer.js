...
/**
 * Send Appointment Reminder on the Day of the Booking
 */
async function sendAppointmentReminderTodayEmail({ to, name, appointment }) {
    const displayName = name ? name.trim() : 'Valued Customer'
    const frontendUrl = getAppUrl()
    ...
    const serviceName = appointment?.service || 'Grooming Visit'
    const haircutStyle = appointment?.haircutStyle ? appointment.haircutStyle : null
    const dateFormatted = appointment?.date || 'Today'
    const timeFormatted = appointment?.time ? `${appointment.time}${appointment.endTime ? ` – ${appointment.endTime}` : ''}` : 'Scheduled Time'
    const priceFormatted = appointment?.price ? `₱${Number(appointment.price).toLocaleString('en-PH')}` : ''

    const subject = `🐾 Reminder: Your appointment for ${petName} is TODAY at ${appointment?.time || ''}`
    const text = `Hi ${displayName},\n\nToday is the day! This is a reminder that ${petName} is scheduled for grooming today at TimmyTails.\n\n⚠️ IMPORTANT NOTICE:\nPlease arrive 5-10 minutes before ${appointment?.time || 'your scheduled time'} or the slot will be automatically cancel and will open to others.\n\nAppointment Details:\n- Pet: ${petName} (${petType}${breed})\n- Service: ${serviceName}${haircutStyle ? ` (${haircutStyle})` : ''}\n- Date: Today (${dateFormatted})\n- Time: ${timeFormatted}\n- Total Price: ${priceFormatted}\n\nSalon Location: Tangos, Baliuag City, Bulacan\n\nView appointment: ${frontendUrl}/appointments\n\nWarmly,\nTimmyTails Team`
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Appointment Today Reminder</title>
</head>
<body style="margin: 0; padding: 0; background-color: #F6F7F2; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #13231B;">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #F6F7F2; padding: 40px 15px;">
        <tr>
            <td align="center">
                <table role="presentation" width="100%" style="max-width: 580px; background-color: #ffffff; border-radius: 24px; border: 1px solid #DDE4DE; overflow: hidden; box-shadow: 0 10px 30px rgba(19, 35, 27, 0.05);" cellspacing="0" cellpadding="0">
                    <!-- Brand Header -->
                    <tr>
                        <td style="background-color: #1F4D3E; padding: 36px 40px; text-align: center;">
                            <h1 style="margin: 0; font-family: Georgia, serif; font-size: 28px; font-weight: bold; color: #F6F7F2; letter-spacing: -0.5px;">🐾 TimmyTails</h1>
                            <p style="margin: 6px 0 0 0; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; color: #A9C6B4;">Same-Day Appointment Reminder</p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding: 40px 40px 30px 40px;">
                            <div style="display: inline-block; background-color: #FFF5F0; border: 1px solid #E06D38; border-radius: 8px; padding: 4px 12px; margin-bottom: 16px;">
                                <span style="font-size: 11px; font-weight: 700; color: #B3471A; text-transform: uppercase; letter-spacing: 1px;">Today's Appointment</span>
                            </div>

                            <h2 style="margin: 0 0 14px 0; font-family: Georgia, serif; font-size: 22px; color: #13231B;">Today is the day, ${displayName}!</h2>
                            <p style="margin: 0 0 20px 0; font-size: 14px; line-height: 1.6; color: #405148;">
                                This is a friendly reminder that <strong>${petName}</strong> is scheduled for grooming today at TimmyTails.
                            </p>

                            <!-- Prominent Warning Notice Box -->
                            <div style="background-color: #FFF5F0; border: 2px solid #E06D38; border-radius: 16px; padding: 18px 20px; margin-bottom: 25px;">
                                <p style="margin: 0 0 6px 0; font-size: 13px; font-weight: bold; color: #B3471A; text-transform: uppercase; letter-spacing: 0.5px;">
                                    ⚠️ Important Salon Policy Notice
                                </p>
                                <p style="margin: 0; font-size: 14px; font-weight: 600; line-height: 1.5; color: #93330C;">
                                    Please arrive 5–10 minutes before ${appointment?.time || 'your scheduled time'} or the slot will be automatically cancel and will open to others.
                                </p>
                            </div>
                            ...
                            <div style="background-color: #FAFBF8; border: 1px solid #E5EAE6; border-radius: 16px; padding: 20px; margin-bottom: 20px;">
                                <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr>
                                        <td style="padding: 6px 0; font-size: 12px; color: #68776F;">Date:</td>
                                        <td style="padding: 6px 0; font-size: 13px; font-weight: bold; color: #1F4D3E; text-align: right;">Today (${dateFormatted})</td>
                                    </tr>
                                    <tr>
                                        <td style="padding: 6px 0; font-size: 12px; color: #68776F;">Time Slot:</td>
                                        <td style="padding: 6px 0; font-size: 13px; font-weight: bold; color: #1F4D3E; text-align: right;">${timeFormatted}</td>
                                    </tr>
                                    ${priceFormatted ? `
                                    <tr>
                                        <td style="padding: 8px 0 0 0; font-size: 12px; font-weight: 700; color: #1F4D3E; border-top: 1px dashed #DDE4DE;">Total Price:</td>
